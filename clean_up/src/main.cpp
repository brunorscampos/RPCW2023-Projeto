...
READ:
        ...
        char* const pos {std::strstr(m_buffer.get(), m_input_rec_sep.c_str())};
        if(pos == nullptr)
            goto READ;


        char const char_at_pos {pos[m_input_rec_sep.size()]};
        pos[m_input_rec_sep.size()] = '\0';

        regmatch_t match_obj[1];
        if(regexec(m_doc_regex_obj.get(), m_buffer.get(), 1, match_obj, 0) != 0)
            return std::nullopt;
        ...
        this->adjust_buffer(pos + m_input_rec_sep.size());
    ...
    while(true){
        ...
        if(j.has_value()){
            ...
            if(j.value().find("Nº do Documento") != j.value().end())
                std::cout << j.value().find("Nº do Documento").value() << '\n';
        }
        else
            break;
    }
